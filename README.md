...
| `SMART_TURN` | TEN VAD pause (`ten-vad.int8.onnx`, bundled) + Smart Turn v3.2 (`@micdrop/smart-turn`) on buffered bot audio, with a max-silence safety cap. Falls back to PSST timer if inference is unavailable. |
...
Optional tuning: `VOIP_SMART_TURN_THRESHOLD`, `VOIP_SMART_TURN_MAX_SILENCE_MS`, `VOIP_SMART_TURN_VAD_MIN_SILENCE_MS`, `VOIP_SMART_TURN_VAD_THRESHOLD`, `VOIP_SMART_TURN_VAD_MODEL_PATH`, `VOIP_SMART_TURN_INFERENCE_TIMEOUT_MS`, `VOIP_SMART_TURN_MIN_COMMIT_DELAY_MS`.
